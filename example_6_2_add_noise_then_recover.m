...
g=ifft2(B.*H'); g=mat2gray(abs(g)); %generate blurred image via %Fourier domain
...
subplot(1,4,1), imshow(A, []); %Display original image
subplot(1,4,2), imshow(g,[min(min(g)) max(max(g))]); %Display original image with noise
